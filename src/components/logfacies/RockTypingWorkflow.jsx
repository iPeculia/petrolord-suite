...
import { Save, Plus, Layers, Download } from 'lucide-react';
...
const RockTypingWorkflow = ({ faciesScheme }) => {
        ...
        <Card className="bg-slate-900 border-slate-800 h-full">
            <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-slate-800">
                ...
                <div className="flex gap-2">
                    <Button size="sm" variant="outline" className="h-8"><Download className="w-3 h-3 mr-2"/> Export R35</Button>
                    <Button size="sm" variant="default" className="h-8 bg-violet-600 hover:bg-violet-500"><Save className="w-3 h-3 mr-2" /> Save Template</Button>
                </div>
            </CardHeader>
            <CardContent className="p-0">
                ...
                <div className="p-4 border-t border-slate-800">
                    <Button variant="ghost" size="sm" className="text-blue-400 hover:text-blue-300" onClick={() => setRockTypes([...rockTypes, { id: Date.now(), facies: 'New', rtype: 'RT-X', phi_avg: 0, k_avg: 0, sw_irr: 0, model: 'Archie' }])}>
                        <Plus className="w-4 h-4 mr-2" /> Add Rock Type definition
                    </Button>
                </div>
            </CardContent>
        </Card>
...
export default RockTypingWorkflow;