...
// --- STAGE MANAGER ---
export const BrownfieldStageManager = ({ tasks }) => {
  const stages = BROWNFIELD_TEMPLATE.stages.map(templateStage => {
  ...
  return (
    <Card className="bg-slate-900 border-slate-800">
        <CardHeader><CardTitle className="text-sm text-slate-300 flex items-center gap-2"><Layers className="w-4 h-4"/> Brownfield Stages</CardTitle></CardHeader>
        <CardContent>
            <Table>
                        ...
                        <TableRow key={idx} className="border-b-slate-800">
                            ...
                            <TableCell>
                                <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                                    <div className="h-full bg-orange-500" style={{ width: `${stage.progress}%` }} />
                                </div>
                            </TableCell>
                            <TableCell>
                                <Badge variant="outline" className={stage.progress === 100 ? "text-green-400 border-green-500/50" : stage.progress > 0 ? "text-orange-400 border-orange-500/50" : "text-slate-500 border-slate-600"}>
...
// --- GATE MANAGER ---
export const BrownfieldGateManager = ({ tasks }) => {
  const gates = tasks.filter(t => t.type === 'milestone' && BROWNFIELD_TEMPLATE.gates.some(g => g.name === t.name));

  return (
    <Card className="bg-slate-900 border-slate-800">
        <CardHeader><CardTitle className="text-sm text-slate-300 flex items-center gap-2"><Flag className="w-4 h-4"/> Approval Gates</CardTitle></CardHeader>
        <CardContent>
             <Table>
                    ...
                    {gates.length > 0 ? gates.map((gate, idx) => (
                        <TableRow key={idx} className="border-b-slate-800">
                            ...
                            <TableCell className="text-right">
                                <Button size="sm" variant="ghost" className="h-7 text-xs text-orange-400 hover:text-white">Criteria</Button>
                            </TableCell>
                        </TableRow>
...
// --- DELIVERABLE MANAGER ---
export const BrownfieldDeliverableManager = ({ deliverables }) => {
  return (
    <Card className="bg-slate-900 border-slate-800">
        <CardHeader><CardTitle className="text-sm text-slate-300 flex items-center gap-2"><FileText className="w-4 h-4"/> Documentation</CardTitle></CardHeader>
        <CardContent>
             <Table>
