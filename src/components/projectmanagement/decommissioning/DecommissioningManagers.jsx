...
export const DecommissioningStageManager = ({ tasks }) => {
  ...
  return (
    <Card className="bg-slate-900 border-slate-800">
        <CardHeader><CardTitle className="text-sm text-slate-300 flex items-center gap-2"><Layers className="w-4 h-4"/> Decommissioning Stages</CardTitle></CardHeader>
        <CardContent>
            <Table>
                        ...
                        <TableRow key={idx} className="border-b-slate-800">
                            ...
                            <TableCell>
                                <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                                    <div className="h-full bg-red-500" style={{ width: `${stage.progress}%` }} />
                                </div>
                            </TableCell>
                            <TableCell>
                                <Badge variant="outline" className={stage.progress === 100 ? "text-green-400 border-green-500/50" : stage.progress > 0 ? "text-red-400 border-red-500/50" : "text-slate-500 border-slate-600"}>
                    ...
                    {gates.length > 0 ? gates.map((gate, idx) => (
                        <TableRow key={idx} className="border-b-slate-800">
                            ...
                            <TableCell className="text-right">
                                <Button size="sm" variant="ghost" className="h-7 text-xs text-red-400 hover:text-white">Criteria</Button>
                            </TableCell>
                        </TableRow>
...
export const DecommissioningDeliverableManager = ({ deliverables }) => {
  return (
    <Card className="bg-slate-900 border-slate-800">
        <CardHeader><CardTitle className="text-sm text-slate-300 flex items-center gap-2"><FileText className="w-4 h-4"/> Key Documentation</CardTitle></CardHeader>
        <CardContent>
             <Table>
