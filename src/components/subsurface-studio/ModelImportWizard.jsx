...
const formatOptions = [
    { value: 'grdecl', label: '.GRDECL' },
    { value: 'grid', label: '.GRID' },
    { value: 'egrid', label: '.EGRID' },
    { value: 'resqml', label: 'RESQML' },
];

const ModelImportWizard = ({ open, onOpenChange, projectId, onImportComplete }) => {
    ...
    const handleImport = async () => {
        ...
        try {
            const filePath = `models/${projectId}/${Date.now()}-${file.name}`;
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="max-w-lg bg-slate-900 border-slate-700 text-white">
                <DialogHeader>
                    ...
                    <DialogDescription>Import corner-point or unstructured grids.</DialogDescription>
                </DialogHeader>
                ...
                <div className="py-4 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input id="model-name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Main Reservoir Model" />
                        </div>
                        <div className="space-y-2">
                            <Label>Format</Label>
                    ...
                    <div {...getRootProps()} className={`p-8 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragActive ? 'border-cyan-400 bg-cyan-900/20' : 'border-slate-600 hover:border-cyan-500 bg-slate-800'}`}>
                        ...
                        <div className="flex flex-col items-center justify-center text-center">
                            ...
                            {file ? (
                                ...
                                <p className="font-semibold text-white">Drag & drop a model file here, or click to select</p>
                            )}
                        </div>
                    </div>
                </div>
