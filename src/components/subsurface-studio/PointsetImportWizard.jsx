...
import { UploadCloud, FileText, Loader2, Milestone } from 'lucide-react';

const formatOptions = [
    { value: 'xyz_ascii', label: 'XYZ ASCII (.xyz, .txt)' },
    { value: 'zmap_plus', label: 'ZMAP+ (.dat)' },
    { value: 'gocad_points', label: 'GoCAD Points (.vs, .vo)' },
];

const PointsetImportWizard = ({ open, onOpenChange, projectId, onImportComplete }) => {
    ...
    const handleImport = async () => {
        ...
        try {
            const filePath = `pointsets/${projectId}/${Date.now()}-${file.name}`;
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="max-w-lg bg-slate-900 border-slate-700 text-white">
                <DialogHeader>
                    <DialogTitle>Import Pointset</DialogTitle>
                    <DialogDescription>Import scattered data points like well tops or seismic picks.</DialogDescription>
                </DialogHeader>

                <div className="py-4 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <Label htmlFor="pointset-name">Name</Label>
                            <Input id="pointset-name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Well Tops" />
                        </div>
                        <div className="space-y-2">
                            <Label>Format</Label>
                    ...
                    <div {...getRootProps()} className={`p-8 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragActive ? 'border-cyan-400 bg-cyan-900/20' : 'border-slate-600 hover:border-cyan-500 bg-slate-800'}`}>
                        ...
                        <div className="flex flex-col items-center justify-center text-center">
                            ...
                            {file ? (
                                ...
                                <p className="font-semibold text-white">Drag & drop a pointset file here, or click to select</p>
                            )}
                        </div>
                    </div>
                </div>
...
export default PointsetImportWizard;