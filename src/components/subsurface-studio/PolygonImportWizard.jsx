...
import { UploadCloud, FileText, Loader2, Shapes } from 'lucide-react';
...
const PolygonImportWizard = ({ open, onOpenChange, projectId, onImportComplete }) => {
    ...
    const handleImport = async () => {
        ...
        try {
            const filePath = `polygons/${projectId}/${Date.now()}-${file.name}`;
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="max-w-lg bg-slate-900 border-slate-700 text-white">
                <DialogHeader>
                    <DialogTitle>Import Polygon</DialogTitle>
                    <DialogDescription>Import polygon data like lease blocks or geological zones.</DialogDescription>
                </DialogHeader>
                ...
                <div className="py-4 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input id="polygon-name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Lease Block A" />
                        </div>
                        <div className="space-y-2">
                            <Label>Format</Label>
                    ...
                    <div {...getRootProps()} className={`p-8 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragActive ? 'border-cyan-400 bg-cyan-900/20' : 'border-slate-600 hover:border-cyan-500 bg-slate-800'}`}>
                        ...
                        <div className="flex flex-col items-center justify-center text-center">
                            ...
                            {file ? (
                                ...
                                <p className="font-semibold text-white">Drag & drop a polygon file here, or click to select</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <DialogFooter>
                    ...
                    <Button onClick={handleImport} disabled={!file || !name || isLoading}>
                        {isLoading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Shapes className="w-4 h-4 mr-2" />}
                        Import Polygon
                    </Button>
                </DialogFooter>
            </DialogContent>
...
export default PolygonImportWizard;