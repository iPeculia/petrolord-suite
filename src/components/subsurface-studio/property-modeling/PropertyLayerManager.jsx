...
const PropertyLayerManager = ({ layers, setLayers }) => {
        ...
        <Card className="bg-slate-900 border-slate-800 shadow-none h-full">
            ...
            <CardContent className="px-4 pt-2">
                ...
                <LayerItem 
                    ...
                    visible={layers.contacts.visible} 
                    opacity={layers.contacts.opacity}
                    onToggle={(v) => updateLayer('contacts', { visible: v })}
                    onOpacityChange={(v) => updateLayer('contacts', { opacity: v })}
                />
                 <LayerItem 
                    id="uncertainty" 
                    label="Uncertainty Bounds" 
                    visible={layers.uncertainty.visible} 
                    opacity={layers.uncertainty.opacity}
                    onToggle={(v) => updateLayer('uncertainty', { visible: v })}
                    onOpacityChange={(v) => updateLayer('uncertainty', { opacity: v })}
                />
            </CardContent>
        </Card>
    );
...
export default PropertyLayerManager;