...
import { Search, Filter, Plus, SortAsc, SortDesc, MoreHorizontal, Eye, EyeOff, Trash2, Edit2 } from 'lucide-react';
...
import { createWellFromLas } from '@/data/wellCorrelation/wellDataModels';
import { useWellManager } from '@/hooks/useWellCorrelation';

const WellListPanel = ({ onWellSelect }) => {
  ...
  const filteredWells = useMemo(() => {
    ...
    // Filter
    if (filterStatus !== 'all') {
      // Assuming well object has status property, defaulting to active if not
  ...
  const handleImport = (importData) => {
    const newWell = createWellFromLas(importData);
    addWell(newWell);
  };
        ...
        <div className="flex gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="bg-slate-900 border-slate-800">
              ...
              <DropdownMenuItem onClick={() => setFilterStatus('archived')} className="text-xs">Archived Only</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <ScrollArea className="flex-1">
        {filteredWells.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-slate-500 text-xs p-4 text-center">
            <p>No wells found.</p>
            <p className="mt-1 opacity-50">Try adjusting filters or import new data.</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onWellSelect && onWellSelect(well)}>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px] text-slate-500">
                    ...
                    <div>{well.depthInfo?.start}-{well.depthInfo?.stop} {well.depthInfo?.unit}</div>
