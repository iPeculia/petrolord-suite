...
import { getAiSuggestions } from '@/utils/wellCostCalculations';

const Section = ({ title, icon, children }) => (
  <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl p-6">
    ...
    <div className="lg:col-span-1 xl:col-span-1 space-y-6">
      ...
      <Section title="Key Cost Drivers" icon={<Wrench />}>
        ...
        <Button onClick={handleGetSuggestions} disabled={suggestionsLoading} variant="outline" className="w-full border-cyan-400/50 text-cyan-300 hover:bg-cyan-500/20">
          ...
          {suggestionsLoading ? "Fetching..." : "Get Suggestions"}
        </Button>
      </Section>
