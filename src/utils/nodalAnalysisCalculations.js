export const generateNodalData = (inputs) => {
        ...
        // Generate OPR/VLP curves (simplified)
        const vlp_current = [];
        const vlp_optimal = [];
        for (let q = 0; q <= aofp; q += aofp / 20) {
            const baseP = 500 + 0.1 * q;
            vlp_current.push({ x: q, y: baseP + 0.05 * q }); // Higher pressure drop for sub-optimal
            vlp_optimal.push({ x: q, y: baseP });
        }

        // Find intersection points (simplified)
        const findIntersection = (iprCurve, vlpCurve) => {
            for (let i = 1; i < iprCurve.length; i++) {
                if (iprCurve[i].y < vlpCurve[i].y) {
                    // Simple linear interpolation
                    const q1 = iprCurve[i-1].x, p_ipr1 = iprCurve[i-1].y, p_vlp1 = vlpCurve[i-1].y;
                    const q2 = iprCurve[i].x, p_ipr2 = iprCurve[i].y, p_vlp2 = vlpCurve[i].y;
                    const q = q1 + (q2 - q1) * (p_vlp1 - p_ipr1) / (p_ipr2 - p_vlp2 - p_ipr1 + p_vlp1);
        ...
        const current_point = findIntersection(ipr, vlp_current);
        const optimal_point = findIntersection(ipr, vlp_optimal);
