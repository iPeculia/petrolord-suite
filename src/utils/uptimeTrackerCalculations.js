...
export const generateUptimeData = (inputs) => {
    ...
    const predictiveData = {
      ...
      runtime: trendData.uptime.map(u => (u / 100) * 24),
      anomalies: trendData.labels.map((_, i) => trendData.uptime[i] < 90 ? { x: trendData.labels[i], y: (trendData.uptime[i] / 100) * 24 } : null).filter(Boolean),
    };
